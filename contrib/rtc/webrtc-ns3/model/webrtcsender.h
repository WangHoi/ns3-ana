...
public:
	WebrtcSender();
	~WebrtcSender();
	void SetMaxRate(uint32_t max_rate);
	typedef Callback<void,uint32_t,uint32_t> TraceRecvFeedback;
	void SetRecvFeedbackTraceFun(TraceRecvFeedback cb){
		m_traceRecvFeedbackCb=cb;
	}
...
private:
	...
	uint32_t m_ssrc{11};//m_uid
	uint32_t m_peerSsrc{12};
    ...
    uint32_t m_recvFeedbackCounter{1};
    TraceRecvFeedback m_traceRecvFeedbackCb;
    webrtc::RtcpBandwidthObserver *m_reportObserver{nullptr};
    webrtc::SequenceNumberUnwrapper m_uint16Toint64;
    WebrtcSeqRecord m_seqRecord;
    uint32_t m_last_sender_report_timestamp{0};
    uint32_t m_maxRate{3000000};
};
}
#endif
