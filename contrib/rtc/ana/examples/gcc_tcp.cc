...
void
WriteLog (int priority, const char *file, int line, const char *fmt, va_list vl)
{
...
BuildExampleTopo (uint64_t bps, uint32_t msDelay, uint32_t msQdelay)
{
  NodeContainer nodes;
  nodes.Create (2);
  ...
  //   pointToPoint.SetQueue ("ns3::DropTailQueue", "Mode", StringValue ("QUEUE_MODE_BYTES"), "MaxBytes",
  //                          UintegerValue (bufSize));
  pointToPoint.SetQueue ("ns3::DropTailQueue", "MaxSize",
                         QueueSizeValue (QueueSize (QueueSizeUnit::BYTES, bufSize)));
...
static void
InstallApps (Ptr<Node> sender, Ptr<Node> receiver, uint16_t port, float startTime, float stopTime)
{
...
main (int argc, char *argv[])
{
  ...
  bwtracer.OpenTraceFile ("gcc_tcp1");
  ...
  InstallTcp (nodes.Get (0), nodes.Get (1), tcpServPort, 10, 40, &bwtracer);
